...
// Ändringar: försöksbaserad berikningslogik (max 3 försök, 1h between retries)
...
function FeedScreen({ user, householdId, household, profiles }) {
  ...
  React.useEffect(function() {
    ...
    var MAX_ATTEMPTS = 3;
    var MIN_RETRY_INTERVAL = 60 * 60 * 1000; // 1 timme mellan försök
    var twoDaysAgo = Date.now() - 48 * 60 * 60 * 1000;
    var now = Date.now();

    var toEnrich = listings.filter(function(l) {
      if (l.enriched === true) return false;                          // redan klar
      if ((l.createdAt || 0) < twoDaysAgo) return false;             // för gammal
      if (!l.url) return false;                                       // ingen URL
      var attempts = l.enrichAttempts || 0;
      if (attempts >= MAX_ATTEMPTS) return false;                     // ge upp
      var lastAttempt = l.enrichedAt || 0;
      if (attempts > 0 && (now - lastAttempt) < MIN_RETRY_INTERVAL) return false; // vänta
      return true;
    });
