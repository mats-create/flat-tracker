...
async function joinHousehold(user, code) {
  ...
  if (snap.empty) {
    throw new Error('Ingen inbjudan hittades med den koden. Kontrollera och försök igen.');
  }
  ...
  if (data.members.length >= 2) {
    throw new Error('Det här hushållet är redan fullt (max 2 medlemmar).');
  }
...
function HouseholdSetupScreen({ user, onComplete }) {
  const [view, setView]       = React.useState('choice');   // choice | create | join | created
  ...
  // ── Hushåll skapat — visa inbjudningskod ───────────────────────────
  if (view === 'created') {
    return (
      <div className="login-screen">
        <div className="login-screen__logo">🎉</div>
